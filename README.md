...
세션이 끝나면 에이전트의 Hook은 transcript 경로만 로컬 SQLite 큐에 넣고 즉시 종료합니다. 단일 백그라운드 worker가 연속 이벤트를 모아 로그를 파싱하고, 코드를 추출하고, 대화를 **세 가지 덩어리 크기로 동시에** 쪼갠 뒤 로컬에서 임베딩합니다(API 호출 없음). 검색 인덱스는 ChromaDB에, 전체 원문은 SQLite에 보존합니다.
...
```
세션 종료 → Hook → SQLite 큐 → 단일 worker → 파싱/쪼개기/임베딩 → ChromaDB + 원문 SQLite
질문         → MCP  → 검색 → 순위 재조정 → 미리보기 → 필요 시 원문 회수
```

Hook은 임베딩 완료를 기다리지 않습니다. 큐 상태와 수동 복구는 `wwt queue-status`, `wwt worker --once`로 확인할 수 있습니다.
