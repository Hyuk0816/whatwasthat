...
WWT는 LLM 대화 로그(JSONL)를 자동으로 파싱하고, 시맨틱 벡터로 변환하여 **자연어로 과거 대화를 검색**할 수 있게 합니다.
...
- **MCP 서버 내장**: Claude Code/Desktop에서 자연어로 바로 검색
- **완전 로컬**: 클라우드 API 호출 없음, 데이터가 내 컴퓨터를 떠나지 않음
- **자동 수집**: Claude Code Stop Hook으로 세션 종료 시 자동 적재
- **원커맨드 설정**: `wwt setup` 한 번으로 DB + Hook + MCP 전부 설정
...
2. Claude Code Stop Hook 스크립트 설치 (세션 종료 시 자동 적재)
3. MCP 서버 글로벌 등록 (`claude mcp add whatwasthat`)
...
`wwt setup` 이후 Claude Code에서 자연어로 바로 사용:

```
사용자: "전에 PostgreSQL 인덱스 최적화 어떻게 했었지?"
Claude: [search_memory 자동 호출] → 관련 대화 3개 찾음
```
...
| `ingest_session` | 대화 로그 적재 (보통 Stop Hook이 자동 처리) |

### 자동 수집 (Stop Hook)

`wwt setup`을 실행하면 Claude Code 세션이 끝날 때마다 대화 로그가 자동으로 적재됩니다. 별도 조작 없이 대화가 쌓입니다.
...
```bash
git clone https://github.com/<owner>/whatwasthat.git
cd whatwasthat
...
| 플랫폼 | 자동 수집 | 검색 | 상태 |
|--------|----------|------|------|
| **Claude Code** | Stop Hook | MCP + CLI | 지원 |
| **Claude Desktop** | - | MCP (기존 DB 검색) | 검색만 가능 |

### 계획 중

| 플랫폼 | 자동 수집 방식 | 로그 포맷 | 예상 |
|--------|--------------|----------|------|
| **Gemini CLI** | AfterAgent Hook | JSON (`contents` 배열) | Phase 3 |
| **Codex CLI** | Stop Hook / 파일 감시 | JSONL (RolloutLine) | Phase 3 |

> 다운스트림(청킹, 임베딩, 검색)은 이미 포맷 독립적입니다. 파서만 확장하면 새 플랫폼을 지원할 수 있습니다.

---

## 로드맵

- [x] **Phase 1**: JSONL 파싱, 청킹, 벡터 임베딩, CLI 검색
- [x] **Phase 2**: MCP 서버, Stop Hook 자동 수집, `wwt setup`
- [ ] **Phase 3**: 크로스 플랫폼 (Gemini CLI, Codex CLI), 파서 추상화, `source` 메타데이터
- [ ] **Phase 4**: PyPI 배포, Docker Compose, 문서 사이트
- [ ] **Phase 5**: `wwt watch` 데몬, Web UI, 모바일 앱
